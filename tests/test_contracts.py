...
def test_analysis_period_rejects_end_before_start() -> None:
    with pytest.raises(ValidationError):
        AnalysisPeriod(start_month=date(2026, 3, 1), end_month=date(2026, 1, 1))


def test_portfolio_question_round_trips() -> None:
...
def test_price_range_rejects_upper_below_lower() -> None:
    with pytest.raises(ValidationError):
        PriceRange(lower_pct=3.0, upper_pct=1.0)


def test_recommendation_requires_evidence_domain_enum() -> None:
    rec = Recommendation(
